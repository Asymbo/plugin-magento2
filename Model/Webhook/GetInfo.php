...
class GetInfo
{
    ...
    public function getInfo($responseData)
    {
        $setting = array(
            'webhooks' => array(
                'address' => $this->storeManager->getStore()->getUrl('asymbo-plugin/webhook/actions/') . 'get-address',
                'stock_count' => $this->storeManager->getStore()->getUrl('asymbo-plugin/webhook/actions/') . 'get-stock-count',
                'save_order' => $this->storeManager->getStore()->getUrl('asymbo-plugin/webhook/actions/') . 'save-order',
                'login' => $this->storeManager->getStore()->getUrl('asymbo-plugin/webhook/actions/') . 'login'
            ),
            'export' => array(
                'products' => $this->storeManager->getStore()->getUrl('asymbo-plugin/export/product'),
                'categories' => $this->storeManager->getStore()->getUrl('asymbo-plugin/export/category')
            ),
            'shipping' => $this->getShipping(),
            'payment' => $this->getPayment(),
            'system' => array(
                'platform' => 'Magento ' . $this->productMetadata->getVersion() . " | " . $this->productMetadata->getEdition(),
                'module_version' => $this->moduleList->getOne('Asymbo_Plugin')['setup_version'],
